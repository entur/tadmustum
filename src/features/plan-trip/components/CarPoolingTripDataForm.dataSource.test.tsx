...
describe('CarPoolingTripDataForm — booking URL tracks the selected data source', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    streetRoute.mockResolvedValue(null);
  });

  it('re-mints the journey code and booking URL when the data source changes on a new trip', async () => {
    const user = userEvent.setup();
    renderForm();

    const bookingUrl = () => screen.getByLabelText('Booking URL') as HTMLInputElement;

    // New trips default to the ENT codespace, so the booking URL is an ENT URL whose
    // embedded code is an ENT:ServiceJourney code.
    await waitFor(() =>
      expect(bookingUrl().value).toMatch(/\/book-trip\/ENT\/ENT:ServiceJourney:/)
    );

    // Switch to the ATB codespace.
...
describe('CarPoolingTripDataForm — data source is locked when editing an existing trip', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    streetRoute.mockResolvedValue(null);
  });

  it("disables the data source picker and keeps the trip's existing code and booking URL", async () => {
    renderForm({ initialState: editingInitialState });

    // The lock explanation replaces the validation message in edit mode.
    await screen.findByText(/data source \(codespace\) is fixed once it's created/i);

    // The picker is rendered (two codespaces) but cannot be changed: changing a
    // saved trip's codespace would orphan it server-side.
    const dataSource = screen.getByRole('combobox', { name: 'Data source' });
    expect(dataSource).toHaveAttribute('aria-disabled', 'true');
